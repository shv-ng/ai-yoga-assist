...
top1 = (
    tf.keras.metrics.sparse_top_k_categorical_accuracy(y_true, y_pred_probs, k=1)
    .numpy()
    .mean()
)
top2 = (
    tf.keras.metrics.sparse_top_k_categorical_accuracy(y_true, y_pred_probs, k=2)
    .numpy()
    .mean()
)
...
print(
    f"TFLite prediction: {le.classes_[np.argmax(tflite_probs)]}  ({max(tflite_probs)*100:.1f}%)"
)
