...
def merge_datasets(existing_csv="data/poses.csv", kaggle_csv="data/kaggle_landmarks.csv", output_csv="data/poses_full.csv"):
    if not os.path.exists(existing_csv):
        print(f"Error: Existing data file {existing_csv} not found.")
        return
    
    if not os.path.exists(kaggle_csv):
        print(f"Error: Kaggle landmarks file {kaggle_csv} not found. Run src/kaggle_to_landmarks.py first.")
        return
